...
public class Q11_JobSorter {
    ...
    private static void merge(
        ...
        while (leftIndex <= mid && rightIndex <= right) {
            if (temp[leftIndex].getPriority() >=
                temp[rightIndex].getPriority()) {
                jobs[target] = temp[leftIndex];
                leftIndex++;
            } else {
                jobs[target] = temp[rightIndex];
                rightIndex++;
            }
            target++;
        }
    ...
    public static Q11_Job findFirstByOwner(
        ...
        for (int index = 0; index < jobs.length; index++) {
            Q11_Job job = jobs[index];
            if (job != null && job.getOwner() != null &&
                job.getOwner().equalsIgnoreCase(owner)) {
                return job;
            }
        }
        return null;
